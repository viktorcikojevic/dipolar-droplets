...
output_dir = "1D_snapshots_x"
if not os.path.exists(output_dir): os.makedirs(output_dir)


while timestep < 2000:
	#print(timestep)
	z1 = np.load(f"{main_dir}/npy_files/psi_equil_{timestep}_x.npy")
	

	zmaxc = np.max(z1) # * 0.01

	plt.plot(x, z1, ls='-', marker='o')
	plt.ylim(0, zmaxc)
	#plt.xlim(np.min(x), np.max(x))
	
	pylab.savefig(f"{output_dir}/1D_snapshot_{timestep}_x.png",  bbox_inches='tight')
	#plt.show()
	pylab.clf()		
	timestep += 1
	plt.close()
