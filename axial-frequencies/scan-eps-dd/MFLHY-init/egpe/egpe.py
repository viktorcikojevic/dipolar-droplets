...
class eGPE:
    ...
    def T2_operator(self, force_symmetry=False):
        """
        ...
        if force_symmetry:
            # flip along x
            self.psi = self.psi + np.flip(self.psi, axis=0)
            # flip along y
            self.psi = self.psi + np.flip(self.psi, axis=1)
            # flip along z
            self.psi = self.psi + np.flip(self.psi, axis=2)
    ...
    def evolve(self,
        ...
        Args:
            ...
            save_x2 (bool): save the x2 density at each print_each_percent percent of the simulation
            force_symmetry (bool): force the wavefunction to be symmetric
        Returns:
            ...
            if i % print_each == 0 or i == 0:
                ...
                if output_root_dir is not None:
                    ...
                    if save_x2:
                        ...
                        x_left = self.x[self.x < 0]
                        x_right = self.x[self.x > 0]
                        den_left = self.den[self.x < 0]
                        den_right = self.den[self.x > 0]
                        peak_den_left = np.max(den_left)
                        peak_den_right = np.max(den_right)

                        distance = np.abs(x_right[np.argmax(den_right)] - x_left[np.argmax(x_left)])
                        # save x2 to x2_file
                        x2_file.write(f'{i * dt} {x2} {peak_den_left} {peak_den_right} {distance}\n')
                        
            
            self.T2_operator(force_symmetry)
