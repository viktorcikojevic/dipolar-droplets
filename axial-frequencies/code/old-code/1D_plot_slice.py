...
nxyz = np.array([128, 128,  128]) / 2     #if you want 1D or 2D, just change this array
...
L = 50. * np.array([1., 1., 1.])  #if you want 1D or 2D, just change this array. This means between -L/2 and L/2
...
while(True):
	print(timestep)
	z = np.load(main_dir+"/npy_files/psi_equil_" + str(timestep) + "_x.npy")
	if(zmaxc < 0.):
		zmaxc = np.max(z)
	zmaxc = np.max(z)
	...
	pylab.savefig("1D_snapshots_slice/1D_snapshot_%i_x" % timestep,  bbox_inches='tight')
	#plt.show()
	pylab.clf()		
	timestep += 1
	plt.close()
